...
- Only waits for a target cursor on spells flagged as needing targeting.
- Reduced target wait timeout to 3 seconds.
- Corrected Flamestrike spell name and added a mana gate before casting.

2026-02-16
- Limited buff-skip logic to spells explicitly configured with a matching `buff` key.

2026-02-17
- Removed buff-skip gating from Necromancy forms (`Horrific Beast`, `Lich Form`) so those casts always run when selected.
- Added Necromancy transition handling to drop active forms at thresholds (`Horrific Beast` at >=70, `Lich Form` at >=95) before casting next-tier spells.
...
def _school_requires_weapon(school):
    if school == "Chivalry":
        return _get_skill_value("Chivalry") < 45
    if school == "Spellweaving":
        return _get_skill_value("Spellweaving") < 44 and SPELL_CAPS["Spellweaving"] > 20
    return False


def _is_target_weapon_equipped():
    global TARGET_WEAPON
    if not TARGET_WEAPON:
        return False
    hand_layers = ["RightHand", "LeftHand", "OneHanded", "TwoHanded"]
    wanted = int(getattr(TARGET_WEAPON, "Serial", 0) or 0)
    for layer in hand_layers:
        try:
            it = API.FindLayer(layer)
        except Exception:
            it = None
        if it and int(getattr(it, "Serial", 0) or 0) == wanted:
            return True
    return False


def _ensure_weapon_equipped_for_school(school):
    global TARGET_WEAPON
    if not _school_requires_weapon(school):
        return True
    if not TARGET_WEAPON:
        return False
    if _is_target_weapon_equipped():
        return True
    try:
        live = API.FindItem(int(getattr(TARGET_WEAPON, "Serial", 0) or 0))
    except Exception:
        live = None
    if not live:
        return False
    API.EquipItem(int(getattr(live, "Serial", 0) or 0))
    API.Pause(0.5)
    return _is_target_weapon_equipped()
...
def _equip_weapon_if_needed(school):
    ...
    if not _school_requires_weapon(school):
        return
    _ensure_weapon_equipped_for_school(school)
...
def _setup_spells():
    if SPELL_CAPS["Magery"] > 0:
        CAST_HOLDER["Magery"] = [
            {"skill": 45, "spell": "Fireball", "wait": 4000, "target": True},
            {"skill": 55, "spell": "Lightning", "wait": 4000, "target": True},
            {"skill": 65, "spell": "Paralyze", "wait": 4000, "target": True},
            {"skill": 75, "spell": "Reveal", "wait": 4000},
            {"skill": 90, "spell": "Flame Strike", "wait": 4000, "target": True},
    ...
    if SPELL_CAPS["Necromancy"] > 0:
        CAST_HOLDER["Necromancy"] = [
            {"skill": 50, "spell": "Pain Spike", "wait": 4000, "target": True},
            {"skill": 70, "spell": "Horrific Beast", "wait": 4000},
            {"skill": 90, "spell": "Wither", "wait": 4000},
            {"skill": 100, "spell": "Lich Form", "wait": 4000},
...
# Ensure a minimum mana threshold before casting.
def _ensure_mana(required):
    if API.Player.Mana >= required:
        return
    while API.Player.Mana < required:
        if API.BuffExists("Meditation"):
            API.Pause(1.0)
        else:
            API.UseSkill("Meditation")
...
# Target self if a target cursor appears.
def _target_self_if_needed(timeout=3.0):
...
# Cast a spell by name and target self when required.
def _cast_spell(school, spell_name, needs_target=False):
    _ensure_weapon_equipped_for_school(school)
    if spell_name == "Flame Strike":
...
# Exit Necromancy forms at tier boundaries, but only if the form buff is active.
def _handle_necromancy_form_transition(skill_val):
    if skill_val >= 95 and API.BuffExists("Lich Form"):
        _cast_spell("Necromancy", "Lich Form")
        API.Pause(4.0)
        _update_gump()
        return True
    if skill_val >= 70 and API.BuffExists("Horrific Beast"):
        _cast_spell("Necromancy", "Horrific Beast")
        API.Pause(4.0)
        _update_gump()
        return True
    return False
...
def _train_skill(school):
    ...
    while skill_val <= skill_cap:
        ...
        _update_gump()
        _check_mana()
        if school == "Necromancy" and _handle_necromancy_form_transition(skill_val):
            continue
        ...
        for spell in cast_list:
            if skill_val < spell["skill"] and skill_val < effective_cap:
                if "buff" in spell and spell["buff"] and API.BuffExists(spell["buff"]):
                    ...
                    casted = True
                    break
                _cast_spell(school, spell["spell"], spell.get("target", False))
